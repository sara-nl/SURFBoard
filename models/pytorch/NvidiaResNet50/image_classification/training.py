...
def get_train_step(model_and_loss, optimizer, fp16, use_amp = False, batch_size_multiplier = 1):
    def _step(input, target, optimizer_step = True):
        ...
        torch.cuda.synchronize()  # sync at the end of forward
        torch.cuda.nvtx.range_pop()  # mark the end of forward
        ...
        if fp16:
            ...
            loss.backward()
        optimizer.synchronize()  # sync Horovod backward explicitly https://horovod.readthedocs.io/en/latest/_modules/horovod/torch/optimizer.html#DistributedOptimizer
        torch.cuda.nvtx.range_pop()  # mark the end of backward

        torch.cuda.nvtx.range_push("Model Update")
        if optimizer_step:
            opt = optimizer.optimizer if isinstance(optimizer, FP16_Optimizer) else optimizer
            for param_group in opt.param_groups:
                for param in param_group['params']:
                    param.grad /= batch_size_multiplier
            with optimizer.skip_synchronize():  # because we are calling optimizer.sync explicitly above already
                optimizer.step()
            optimizer.zero_grad()

        torch.cuda.synchronize()  # sync weight update
        torch.cuda.nvtx.range_pop()  # mark end of update
...
def train(train_loader, model_and_loss, optimizer, lr_scheduler, fp16, logger, epoch, use_amp=False, prof=-1, batch_size_multiplier=1, register_metrics=True):
    ...
    for i, (input, target) in data_iter:
        ...
        torch.cuda.nvtx.mark("End of Batch")
