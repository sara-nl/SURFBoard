...
def add_parser_arguments(parser):
    ...
    parser.add_argument('--dataidx', default=None, metavar='DIR',
                        help='path to dataset index')
...
def main(args):
    ...
    if args.distributed:
        args.gpu = hvd.local_rank() % torch.cuda.device_count()
    ...
    if args.seed is not None:
        ...
        np.random.seed(seed=args.seed + hvd.rank())
        random.seed(args.seed + hvd.rank())

        def _worker_init_fn(id):
            np.random.seed(seed=args.seed + hvd.rank() + id)
            random.seed(args.seed + hvd.rank() + id)
    else:
        def _worker_init_fn(id):
            pass
    ...
    if args.data_backend == 'pytorch':
        ...
        get_train_loader = get_dali_train_loader(dali_cpu=False, to_gpu=True)
        get_val_loader = get_dali_val_loader(dali_cpu=False, to_gpu=True)
    elif args.data_backend == 'dali-cpu':
        get_train_loader = get_dali_train_loader(dali_cpu=True, to_gpu=False)
        get_val_loader = get_dali_val_loader(dali_cpu=True, to_gpu=False)
    elif args.data_backend == 'dali-cpu-to-gpu':
        get_train_loader = get_dali_train_loader(dali_cpu=True, to_gpu=True)
        get_val_loader = get_dali_val_loader(dali_cpu=True, to_gpu=True)

    train_loader, train_loader_len = get_train_loader(args.data, args.dataidx, args.batch_size, 1000, args.mixup > 0.0, workers=args.workers, fp16=args.fp16)
    if args.mixup != 0.0:
        train_loader = MixUpWrapper(args.mixup, 1000, train_loader)

    val_loader, val_loader_len = get_val_loader(args.data, args.dataidx, args.batch_size, 1000, False, workers=args.workers, fp16=args.fp16)
    ...
    hvd_fp16_compression = hvd.Compression.fp16 if args.fp16_allreduce else hvd.Compression.none

    optimizer = hvd.DistributedOptimizer(optimizer, named_parameters=model_and_loss.model.named_parameters(),compression=hvd_fp16_compression)
